...
type Payslip = typeof payslips[0]
...
export function LonesbeskContent() {
    ...
    const stats = useMemo(() => {
        // Get unique periods and find the latest one
        const periods = [...new Set(payslips.map(p => p.period))]
        const currentPeriod = periods[0] // First is most recent

        // Filter payslips for current period
        ...
        // Calculate deadline (12th of next month)
        const periodMatch = currentPeriod.match(/(\w+)\s+(\d{4})/)
    ...
    const toggleAll = () => {
        if (selectedIds.size === payslips.length) {
            ...
            setSelectedIds(new Set(payslips.map(p => p.id)))
        }
    }
    ...
    const bulkActions: BulkAction[] = [
        {
            ...
            variant: "destructive",
            onClick: (ids) => {
                toast.success("Lönespecifikationer borttagna", `${ids.length} specifikation(er) har tagits bort`)
                setSelectedIds(new Set())
            },
        },
        {
            id: "send",
            label: "Skicka till anställd",
            icon: Send,
            onClick: (ids) => {
                toast.success("Lönespecifikationer skickade", `${ids.length} specifikation(er) skickades till anställda`)
                setSelectedIds(new Set())
            },
        },
        {
            ...
            icon: Download,
            onClick: (ids) => {
                toast.info("Laddar ner", `Förbereder nedladdning av ${ids.length} specifikation(er)...`)
                setSelectedIds(new Set())
            },
        },
    ]
                ...
                >
                    <DataTableHeader>
                        <DataTableHeaderCell className="w-10">
                            <Checkbox
                                checked={selectedIds.size === payslips.length && payslips.length > 0}
                            ...
                            >
                                <DataTableCell className="w-10">
                                    <Checkbox
                                        ...
                                        onCheckedChange={() => toggleSelection(slip.id)}
