...
export default function SkatteverketPage() {
  ...
  useEffect(() => {
    ...
    setSubmissions(allSubmissions.filter(s => s.agency === 'skatteverket'))
  }, [])

  useEffect(() => {
    // const interval = setInterval(() => {
    //   const allSubmissions = getStoredSubmissions()
    //   const skatteverket = allSubmissions.filter(s => s.agency === 'skatteverket')
    //   if (skatteverket.length !== submissions.length) {
    //     setSubmissions(skatteverket)
    //   }
    // }, 2000)
  ...
  return (
    <div className="container max-w-5xl py-8 px-4">
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-2">🏛️ Skatteverket</h1>
        <p className="text-gray-600">
          Ärenden skickade till Skatteverket från din app. AI validerar dokumenten automatiskt.
        </p>
      ...
      {submissions.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center text-gray-500">
            ...
            <p>Inga ärenden hos Skatteverket</p>
            <p className="text-sm mt-2">Skicka in AGI, moms eller andra dokument från Dashboard.</p>
          </CardContent>
        </Card>
      ) : (
