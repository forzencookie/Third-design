...
export default function BolagsverketPage() {
  ...
  useEffect(() => {
    ...
    setSubmissions(allSubmissions.filter(s => s.agency === 'bolagsverket'))
  }, [])

  useEffect(() => {
    // const interval = setInterval(() => {
    //   const allSubmissions = getStoredSubmissions()
    //   const bolagsverket = allSubmissions.filter(s => s.agency === 'bolagsverket')
    //   if (bolagsverket.length !== submissions.length) {
    //     setSubmissions(bolagsverket)
    //   }
    // }, 2000)
  ...
  return (
    <div className="container max-w-5xl py-8 px-4">
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-2">🏢 Bolagsverket</h1>
        <p className="text-gray-600">
          Ärenden skickade till Bolagsverket från din app.
        </p>
      </div>
      ...
      {submissions.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center text-gray-500">
            ...
            <p>Inga ärenden hos Bolagsverket</p>
            <p className="text-sm mt-2">Skicka in årsredovisning eller andra dokument från Dashboard.</p>
          </CardContent>
        </Card>
      ) : (
