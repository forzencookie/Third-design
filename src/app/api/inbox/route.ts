...
// Simple in-memory storage (Now using shared DB)
import { mockDB } from "@/data/mock-db"
import { randomUUID } from "crypto"

export async function GET() {
    return NextResponse.json({
        items: mockDB.inboxItems
    })
}
...
export async function POST(request: Request) {
    try {
        ...
        if (action === 'create' && type) {
            // Create a specific type of message (RAW data only, no AI processing here)
            const newItem: InboxItem = {
                ...
                category: "other",
                aiStatus: 'pending', // Will be processed by AI engine separately
                ...(generateMockItem(type) as any)
            }

            // NO AI processing here - that happens via /api/ai/process-inbox
